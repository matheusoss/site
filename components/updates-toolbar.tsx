...
export function UpdatesToolbar({ posts }: { posts: any[] }) {
  ...
  const handleOnShare = () => {
    const popup = popupCenter({
      url: `https://twitter.com/intent/tweet?text=${currentPost.title} https://matheusoss.com/blog/${currentPost.slug}`,
        ...
        <TooltipProvider delayDuration={20}>
          <div className="flex flex-col items-center backdrop-filter backdrop-blur-lg dark:bg-[#1A1A1A]/80 p-2 border dark:border-[#2C2C2C] bg-white space-y-4 rounded-full">
            <Tooltip>
              ...
              <TooltipContent className="py-1 px-3 rounded-sm" sideOffset={25} side="right">
            ...
            <div className="flex flex-col items-center border-t-[1px] border-border space-y-2 pt-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" className={cn(currentIndex === 0 && 'opacity-50')} onClick={handlePrev}>
                    <Icons.ChevronUp className="h-6 w-6" />
                  </button>
                </TooltipTrigger>
                <TooltipContent className="py-1 px-3 rounded-sm" sideOffset={25} side="right">
                  <span className="text-xs">Post Anterior</span>
                </TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    type="button"
                    className={cn(currentIndex === posts.length - 1 && 'opacity-50')}
                    onClick={handleNext}
                  >
      ...
      <DialogContent className="sm:max-w-[425px]">
        <div className="p-6">
          ...
          <div className="grid gap-6 py-4">
            <CopyInput value={`https://brokerpe.com${pathname}`} />
